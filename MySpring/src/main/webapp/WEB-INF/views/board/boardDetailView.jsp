				...
				<div class="col-lg-8 col-xl-7">
					...
					<div>
						<div class="board-head">
							...
							<div class="name-date">
								<div class="board-name">${keyBoard.memName}</div>
								<div class="board-date">${keyBoard.boardDate} </div>
							</div>
						</div>
						
						<div class="d-flex justify-content-end">
							<b>조회수 : ${keyBoard.boardCount }</b>
						</div>
						
						
						<!-- 첨부파일 목록 표기 -->
						<div>
							<c:forEach items="${keyAttachList}" var="attachDTO">
								<c:if test="${attachDTO.atchOriginalName != null }">
									<!-- 클릭 시 현재 첨부파일의 실제 저장된 이름을 서버에 요청 -->
									<a href="${pageContext.request.contextPath }/fileDownload?fileName=${attachDTO.atchFileName}&originName=${attachDTO.atchOriginalName}">${attachDTO.atchOriginalName }[${attachDTO.atchFancySize }]</a>
								</c:if>
								<c:if test="${attachDTO.atchOriginalName == null }">
								</c:if>
							</c:forEach>
						...
						<div class="board-body">${ keyBoard.boardContent}</div>
					...
					<!-- 일치하면 글수정 버튼 그리기 -->
					<div class="d-flex justify-content-end">
						<c:if test="${ sessionScope.login.memId == keyBoard.memId && sessionScope.login.memId != null}">
						...
						<c:if test="${ sessionScope.login.memId == keyBoard.memId && sessionScope.login.memId != null || sessionScope.login.memLv == '0'}">
